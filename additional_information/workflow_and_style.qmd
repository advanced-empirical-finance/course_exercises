...
prices |> 
  ...
  ggplot(aes(x = date, y = mean_adjusted)) +
  geom_point() + 
  geom_line()
...
prices %>%
  group_by(date) %>%
  summarize(
    mean_high_low = mean(c(high, low), na.rm = TRUE)
  ) %>%
  ggplot(aes(x = date, y = mean_high_low)) +
  geom_point() + 
  geom_line()
...
from plotnine.stats import mean_sdl
from plotnine import ggtitle

prices
        .groupby('date')
        .agg(mean_high_low=('high', 'mean'))
        .reset_index()
        .pipe(ggplot, aes(x='date', y='mean_high_low'))
        + geom_point()
        + geom_line())
...
# Group by date and calculate the mean of 'adjusted'
summary_data = (prices.groupby('date')
                 .agg(mean_adjusted=('adjusted', 'mean'))
                 .reset_index())

# Create the ggplot
plot = (ggplot(summary_data, aes(x='date', y='adjusted'))
        + geom_point()
